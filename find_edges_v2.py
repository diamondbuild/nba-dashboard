...
# ============================================================================
# EDGE THRESHOLDS (Based on 68.8% win rate analysis)
# ============================================================================
MIN_EDGE = 2.0      # Minimum edge to consider (2-4 range = 81.8% win rate)
MAX_EDGE = 6.0      # Maximum edge to consider (6+ range = unreliable)
ONLY_OVERS = True   # Only bet OVERS to avoid OT disasters
...
def get_injury_report():
    ...
    Returns dict of injured players by team
    """
    try:
...
for stat in ['PTS', 'REB', 'AST', '3PM', 'PRA', 'STL', 'BLK']:
    ...
    for _, row in merged.iterrows():
        ...
        # ONLY OVERS within edge threshold
...
# === STEP 6: SAVE & EMAIL ===
print("\n[6/6] Saving results and sending email...")
...
if edges:
    ...
    <body style="font-family: Arial, sans-serif;">
        <h2 style="color: #2c3e50;">NBA Props: {len(edges)} OVER Edges (v2.0)</h2>

        <div style="background-color: #27ae60; color: white; padding: 15px; border-radius: 5px; margin: 20px 0;">
            <h3 style="margin: 0;">{len(edges)} OVER Bets Found</h3>
            <p style="margin: 5px 0;">Context-adjusted | OT-proof strategy</p>
        </div>

        <h3>Edge Distribution:</h3>
        <ul>
            <li><strong>Edge 2-3:</strong> {len(edges_df[(edges_df['EDGE'] >= 2) & (edges_df['EDGE'] < 3)])}</li>
            <li><strong>Edge 3-4:</strong> {len(edges_df[(edges_df['EDGE'] >= 3) & (edges_df['EDGE'] < 4)])}</li>
            <li><strong>Edge 4-6:</strong> {len(edges_df[(edges_df['EDGE'] >= 4) & (edges_df['EDGE'] <= 6)])}</li>
        </ul>

        <h3>Top 10 Edges:</h3>
        {edges_df.head(10).to_html(index=False, border=1)}

        <p style="color: #7f8c8d; margin-top: 30px;">
            <small>Generated: {datetime.now().strftime('%I:%M %p EST on %B %d, %Y')}</small>
        </p>
    </body>
    </html>
    """

    email_subject = f"NBA Props: {len(edges)} OVER Edges - {datetime.now().strftime('%b %d')}"
else:
    ...
    <html>
    <body style="font-family: Arial, sans-serif;">
        <h2>NBA Props: No Edges Today</h2>
        <p>No OVER bets met the criteria (edge between {MIN_EDGE} and {MAX_EDGE}).</p>
        <p><small>{datetime.now().strftime('%I:%M %p EST')}</small></p>
    </body>
    </html>
    """
